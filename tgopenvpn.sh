...
export PATH
# wget http://www.moli52.com/vpn/xsopenvpn.sh && bash xsopenvpn.sh 2>&1 | tee xsopenvpn.log
clear;
...
==========================================================================
                       ...
                       CentOS  OpenVPN-2.3.14 云免服务器                                  
                         Powered by 涛哥 2015-2016                     
                              All Rights Reserved                  
...
echo "$CopyrightLogo";
echo 
echo "脚本已由阿里云/腾讯云CentOS/网易云6.7 测试通过"
echo "如有不懂，联系涛哥的Blog QQ3500867"
echo "请按回车继续开始安装："
read
echo 
# xiaosen
...
rpm -ivh http://www.moli52.com/vpn/64epel-release-5-4.noarch.rpm
else
rpm -ivh http://www.moli52.com/vpn/32-epel-release-5-4.noarch.rpm
fi
fi

if [ $version == "6" ];then
if [ $(getconf LONG_BIT) = '64' ] ; then
rpm -ivh http://www.moli52.com/vpn/epel-release-6-8.noarch.rpm
else
rpm -ivh http://www.moli52.com/vpn/32-epel-release-6-8.noarch.rpm
fi
fi

if [ $version == "7" ];then
rpm -ivh http://www.moli52.com/vpn/epel-release-latest-7.noarch.rpm
fi
...
cd /etc/
rm -rf ./sysctl.conf
wget http://www.moli52.com/vpn/sysctl.conf
sleep 3
...
cd /etc/openvpn/
rm -rf ./server.conf
wget http://www.moli52.com/vpn/server.tar.gz
tar -zxf server.tar.gz
cd /etc/squid/
rm -f ./squid.conf
wget http://www.moli52.com/vpn/squid.conf
...
echo "获取共享转发端口..."
sqip=`wget http://www.moli52.com/vpn/ips -O - -q ; echo`
# OpenVPN Installing ****************************************************************************
cd /etc/openvpn/
wget http://www.moli52.com/vpn/EasyRSA-2.2.2.tar.gz
